...
@app.route('/consultarDiagnostico', methods=['POST'] )
def mostrarDiagnostico():
    _Paciente_expediente = request.form['expBus']
    conn = mysql.connect()
    cursor = conn.cursor(pymysql.cursors.DictCursor)
    cursor.execute("SELECT * FROM diagnostico WHERE idDiagnostico =%s", _Paciente_expediente)                                                                                                                                     
    data1 = cursor.fetchone()                                                                                     
    return render_template('tasks3.html', d = data1)


@app.route('/consultarVisita', methods=['POST'] )
def mostrarVisita():
    _Paciente_expediente = request.form['expBus']
    conn = mysql.connect()
    cursor = conn.cursor(pymysql.cursors.DictCursor)
    cursor.execute("SELECT * FROM visita WHERE idDiagnostico =%s", _Paciente_expediente)                                                                                                                                     
    data1 = cursor.fetchone()                                                                                     
    return render_template('tasks4.html', d = data1)
...
def tasks1():
    ...
    data = cursor.fetchall()    
    return render_template('tasks.html', contacts1 = data)
...
def tasks2():
    ...
    data = cursor.fetchall()    
    return render_template('tasks.html', contacts2 = data)
